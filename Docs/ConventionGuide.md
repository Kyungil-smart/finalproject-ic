...
- 작업 완료 후 Merge 진행
  - Merge한 브랜치는 되도록 삭제. 단, 계속 진행하는 브랜치면 다 완료된 후 삭제.
...
Log 는 아래 양식에 맞게 적어 주시기 바랍니다.

### Log 남기기
1. 단순한 데이터 확인용은 지양. 단, 복잡한 데이터 구조나 확인이 필요한 것들은 이벤트성으로 남길 수 있음.
2. 반복문 내에 Log 남기는 것은 지양함.

### Log Level 에 맞춰 적기
1. Log: 일반적인 정보 표기
2. Warning: 게임에 문제는 없지만 추후 영향이 있을 수 있는 내용
3. Error: 게임 진행에 문제가 발생할 수 있는 내용
   
### Format
```csharp
Debug.Log("[클래스 이름] 관련내용 적기");
// 클래스 이름: 어디서 발생된 로그인지 확인이 필요하기 때문. Editor 에서는 잘 보이는데 Build 후에는 안보이기 때문에 명시적으로 적어줄 것.
// 관련 내용: 단순한 데이터 확인용은 지양. 
```